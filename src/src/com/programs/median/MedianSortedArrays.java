...
public class MedianSortedArrays {
    public static void main(String[] args) {
        ...
        System.out.println(findMedian(new int[]{9, 10, 11, 12, 13, 14}, 0, 5, new int[]{1, 2, 3, 4, 5, 6, 7, 8}, 0, 7));
    }
    ...
    public static double findMedian(int[] ar1, int s1, int e1, int[] ar2, int s2, int e2) {
        ...
        if (m1 < m2) {
            ...
            return recurse(ar2, s2, e2, ar1, s1, e1);
        }
    }
    ...
    private static double recurse(int[] ar1, int s1, int e1, int[] ar2, int s2, int e2) {
        ...
        if (l1 <= l2) {
            countRemovedElements = (int) Math.ceil((double) l1 / 2);
        } else {
            countRemovedElements = (int) Math.ceil((double) l2 / 2);
        }
        return findMedian(ar1, s1 + countRemovedElements, e1, ar2, s2, e2 - countRemovedElements);
    }
